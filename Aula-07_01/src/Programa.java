...
public class Programa {
	...
	public static void main(String args[]) throws ParseException {
		...
		for(int i = 0; i < 6; i++) {
			conjuntoDeContinente.add(new Continente(nomeDosContinentes[i]));
		...
		while(controle != -1) {
			...
			if(id != 0) {
				...
				Pais pais = new Pais(nomePais, populacao, dimensao);
		...
		try(BufferedWriter bw = new BufferedWriter(new FileWriter(caminho, true))){
			for(int y = 0; y < conjuntoDeContinente.size(); y++) {
				bw.write(conjuntoDeContinente.get(y).relatorio().toUpperCase());
				bw.newLine();
			}
		} catch (IOException e) {
			e.printStackTrace();
